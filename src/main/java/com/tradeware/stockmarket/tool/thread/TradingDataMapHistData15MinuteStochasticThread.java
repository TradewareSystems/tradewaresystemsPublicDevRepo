...
public class TradingDataMapHistData15MinuteStochasticThread extends AdditionalDataFillupHelper {
	...
	public void run() {
		...
		if (TradewareLogger.isDebugLevelEnable) {
			...
			TradewareLogger.saveInfoLog(
					CLASS_TRADING_DATA_MAP_HIST_DATA_STC_15_MINUTE_THREAD + " [" + super.threadName + "]", METHOD_RUN,
		...
		if (TradewareLogger.isDebugLevelEnable) {
			...
			TradewareLogger.saveInfoLog(CLASS_TRADING_DATA_MAP_HIST_DATA_STC_15_MINUTE_THREAD + " ["+super.threadName+"]", METHOD_RUN,
	...
	private void initGet60MinuteCandleData() {
		...
		try {
			if (TradewareLogger.isDebugLevelEnable) {
				...
				TradewareLogger.saveInfoLog(
						CLASS_TRADING_DATA_MAP_HIST_DATA_STC_15_MINUTE_THREAD + " [" + super.threadName + "]",
	...
	private void repaeat15MinutueCandleDataFull(ConcurrentHashMap<String, StrategyOrbDataBean> dataMap) {
		try {
			...
			try {
				TradewareLogger.saveInfoLog(
						CLASS_TRADING_DATA_MAP_HIST_DATA_STC_15_MINUTE_THREAD + " [" + super.threadName + "]" + dummyKey,
			...
			if (!(null == dataMap || dataMap.isEmpty() || null == fromDate || null == toDate)) {
				for (String key : dataMap.keySet()) {
					...
					if (!processedSymbols.contains(key)) {
						...
						findStochasticTrend( histDataList,  avgBean, KITE_HIST_DATA_15_MINUTES_INTERVAL);
						findHeikinAshiTrend( histDataList,  avgBean, KITE_HIST_DATA_15_MINUTES_INTERVAL);
						}
				}
			} else {
				TradewareLogger.saveInfoLog(
