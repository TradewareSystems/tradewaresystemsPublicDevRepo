...
public class TradingDataMapHistData5MinuteStochasticThread extends AdditionalDataFillupHelper {
	...
	public void run() {
		...
		if (TradewareLogger.isDebugLevelEnable) {
			...
			TradewareLogger.saveInfoLog(
					CLASS_TRADING_DATA_MAP_HIST_DATA_STC_5_MINUTE_THREAD + " [" + super.threadName + "]", METHOD_RUN,
		...
		if (TradewareLogger.isDebugLevelEnable) {
			...
			TradewareLogger.saveInfoLog(CLASS_TRADING_DATA_MAP_HIST_DATA_STC_5_MINUTE_THREAD + " ["+super.threadName+"]", METHOD_RUN,
	...
	private void initGet60MinuteCandleData() {
		...
		try {
			if (TradewareLogger.isDebugLevelEnable) {
				...
				TradewareLogger.saveInfoLog(
						CLASS_TRADING_DATA_MAP_HIST_DATA_STC_5_MINUTE_THREAD + " [" + super.threadName + "]",
			...
			repaeat15MinutueCandleDataFull(tradewareTool.getBaseDataMap());
			repaeat15MinutueCandleDataFull(tradewareTool.getBaseDataMapAll());
			if (TradewareLogger.isDebugLevelEnable) {
				long exit = System.currentTimeMillis();
				TradewareLogger.saveInfoLog(
						CLASS_TRADING_DATA_MAP_HIST_DATA_STC_5_MINUTE_THREAD + " [" + super.threadName + "]",
	...
	private void repaeat15MinutueCandleDataFull(ConcurrentHashMap<String, StrategyOrbDataBean> dataMap) {
		try {
			if (null != dataMap && !dataMap.isEmpty()) {
				get15MinutueCandleDataFull(dataMap);
			}
		} catch (KiteException | Exception e) {
			processedCounter++;
			try {
				TradewareLogger.saveInfoLog(
						CLASS_TRADING_DATA_MAP_HIST_DATA_STC_5_MINUTE_THREAD + " [" + super.threadName + "]" + dummyKey,
			...
			if (dataMap.size() != processedSymbols.size() && processedCounter <= 100) {
				repaeat15MinutueCandleDataFull(dataMap);
			}
		}
	}
	...
	// Should not write try/catch, repaeat5MinutueCandleDataFull will catch the
		...
		private void get15MinutueCandleDataFull(ConcurrentHashMap<String, StrategyOrbDataBean> dataMap) throws JSONException, IOException, KiteException, NetworkException {
			...
			if (!(null == dataMap || dataMap.isEmpty() || null == fromDate || null == toDate)) {
				for (String key : dataMap.keySet()) {
					...
					if (!processedSymbols.contains(key)) {
						...
						findStochasticTrend( histDataList,  avgBean, KITE_HIST_DATA_5_MINUTES_INTERVAL);
						findHeikinAshiTrend( histDataList,  avgBean, KITE_HIST_DATA_5_MINUTES_INTERVAL);
						}
				}
			} else {
				TradewareLogger.saveInfoLog(
						...
						for (String key : dataMap.keySet()) {
							...
							if (!processedSymbols.contains(key)) {
								if (KiteConnectTool.minute5KiteHistDataMap.contains(key)) {
									...
									histData = StockUtil.kiteConnectAdminsHist.get(StockUtil.DEFAULT_USER)
											.getHistoricalData(fromDate, toDate, String.valueOf(dataMap.get(key).getInstrumentToken()),
													KITE_HIST_DATA_5_MINUTES_INTERVAL, false);
